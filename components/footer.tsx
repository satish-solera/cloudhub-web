    ...
    <div className="min-h-svh ml-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
        ...
        {FooterData.map((el, id) => {
          return <FooterBottom key={id} title={el.title} list={el.list} />;
        })}
      </div>
      <p className="mx-auto my-5 text-center text-[#5F6980] text-[14px]">
        All rights reserved.© 2026 vengenceui
      </p>
    </div>
  );
};
    ...
    <div className="">
      ...
      <ul>
        {list.map((el, id) => {
          return (
            <li
              className="text-[14px] text-[#647192] my-2 cursor-pointer hover:text-[#1a2135] hover:text-[15px] transition-all"
              key={id}
            >
              {el}
            </li>
